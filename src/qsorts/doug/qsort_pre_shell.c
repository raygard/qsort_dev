...
void qsort(char *a, size_t n, size_t es, int (*cmp)())
{
	...
	int r, swaptype, swap_flag;
	long t, v;
	size_t s;

    /* Quit if all in order */
    for (pm = a + es; pm < a + n * es && cmp(pm - es, pm) <= 0; pm += es)
        ;
    if (pm == a + n * es)
        return;
	...
	for (;;) {
		while (pb <= pc && (r = cmp(pb, pv)) <= 0) {
			if (r == 0) { swap_flag = 1; swap(pa, pb); pa += es; }
			pb += es;
		}
		while (pb <= pc && (r = cmp(pc, pv)) >= 0) {
			if (r == 0) { swap_flag = 1; swap(pc, pd); pd -= es; }
			pc -= es;
		}
		if (pb > pc) break;
		swap_flag = 1;
	...
	if (swap_flag == 0) {  /* Switch to Shell sort */
        size_t gap;
        for (gap = 4; gap < n; gap = 3 * gap + 1)
            ;
        while (gap /= 3) {
            for (pm = a + gap * es; pm < a + n * es; pm += es)
                for (pl = pm; pl >= a + gap * es && cmp(pl - gap * es, pl) > 0;
                        pl -= gap * es)
                    swap(pl, pl - gap * es);
            }
		return;
